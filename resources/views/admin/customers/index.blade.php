...
<div class="page-header mb-4">
    <h2 class="fw-semibold text-dark">🔄 Refund Requests</h2>
</div>
                ...
                <tr>
                    ...
                    <td>
                        @if($refund->refund_status === 'requested')
                            <form action="{{ route('admin.refunds.update', $refund) }}" method="POST" class="d-flex flex-column gap-1">
                                @csrf
                                @method('PUT')
                                <button type="submit" name="status" value="approved" class="btn btn-sm btn-success" title="Setujui Refund">
                                    <i class="bi bi-check-circle"></i> Setuju
                                </button>
                                <button type="submit" name="status" value="rejected" class="btn btn-sm btn-danger" title="Tolak Refund">
